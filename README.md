...
本專案開發了一個基於 Stable Diffusion 1.5 與自訂 LoRA 模型 的 Chiikawa 角色生成器，並使用 Streamlit 提供互動式前端介面。專案目標在於透過少量資料（約 500 張 Chiikawa 角色圖片）訓練 LoRA，以便生成風格統一、可愛生動的角色圖像。使用者可自由調整生成參數，包括推理步數（Steps）、提示強度（Guidance Scale）、批量生成張數（Batch Size）及 LoRA 權重，從而控制生成圖像的細節、貼合度與風格強度。此外，系統支援多個自訂 LoRA 模型(需自行訓練)，使用者可根據需求切換角色風格。由於生成模型與 LoRA 模型檔案容量過大，本專案以 Streamlit 網頁提供即時互動生成，而最終結果可透過截圖或影片方式呈現。此工具可應用於角色創作、動漫插畫以及個人化圖像生成研究，並提供使用者簡單易懂的操作介面，降低生成高品質動漫角色圖像的技術門檻。實驗結果顯示，結合少量圖片訓練的 LoRA 模型能有效捕捉 Chiikawa 的風格特徵，生成圖像表現還行，且操作過程直觀方便，符合快速生成與風格化圖像需求。
